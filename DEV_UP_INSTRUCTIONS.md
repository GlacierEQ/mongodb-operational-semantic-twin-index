...
**Company lens (independent):** MongoDB (`mongodb`)  
**Innovation:** Operational-Semantic Twin Index  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim MongoDB affiliation, proprietary access, or production deployment.
...
Derive semantic indexes directly from authoritative document mutations with provenance and transaction linkage so app state and retrieval state cannot silently diverge.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply MongoDB employment, endorsement, or proprietary systems access.
...
File: `src/operational_semantic_twin_index.py`

- Expand `OperationalSemanticTwinIndex` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_operational_semantic_twin_index.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_operational_semantic_twin_index.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary MongoDB APIs
